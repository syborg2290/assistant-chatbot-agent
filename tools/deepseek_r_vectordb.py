...
class DeepSeekVectorDB:
    """Handles AI-powered querying using DeepSeek and a vector database."""
    ...
    def _initialize_qa_chain(self):
        """Creates an AI assistant using DeepSeek and LangChain with current parameters."""
        template = """<｜begin▁of▁sentence｜>[INST] <<SYS>>
        ...
        llm = OllamaLLM(
            model=config.DEEPSEEK_LLM_OLLAMA,
    ...
    def _process_response(answer):
        ...
        reasoning = re.findall(r"<think>(.*?)</think>", answer, flags=re.DOTALL)
        return {
...
# Singleton instance
deepseek_vector_db = DeepSeekVectorDB()
